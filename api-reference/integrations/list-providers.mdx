---
title: List available providers
api: "GET /integrations/providers"
description: List every provider you can connect, grouped by channel
---

Returns the providers available on the platform grouped by channel — useful for
rendering a "connect a provider" picker, or for discovering which telephony
providers you can register numbers against.

### Query parameters

<ParamField query="channel" type="string">
  Filter to a single channel, e.g. `VOICE`, `WHATSAPP`, `SMS`
</ParamField>

<CodeGroup>

```bash curl
curl "https://dev-agentic-os-platform-core-api-aks.setoo.work/api/v1/integrations/providers?channel=VOICE" \
  -H "Authorization: Bearer $AGENTIC_OS_API_KEY"
```

```python Python
requests.get(
    "https://dev-agentic-os-platform-core-api-aks.setoo.work/api/v1/integrations/providers",
    headers={"Authorization": f"Bearer {API_KEY}"},
    params={"channel": "VOICE"},
)
```

```javascript Node
await fetch(
  "https://dev-agentic-os-platform-core-api-aks.setoo.work/api/v1/integrations/providers?channel=VOICE",
  { headers: { Authorization: `Bearer ${API_KEY}` } },
);
```

</CodeGroup>

### Response

```json
[{ "channel": "VOICE", "providers": ["TWILIO", "PLIVO", "ACEFONE"] }]
```
