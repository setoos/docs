...
Sign in to your platform console (e.g. `http://localhost:8080` locally, or your deployed Dev / UAT / Prod URL) with your business account. For more information on configuring your organization and keys, see the [API Keys Guide](/features/api-keys).

## 2. Create an API key

<Steps>
  <Step title="Open the API Keys page">
    In the dashboard sidebar, go to **API Keys** (or see the [API Keys Guide](/features/api-keys)).
  </Step>
...
```bash curl
curl https://dev-agentic-os-aks.setoo.work/api/v1/agents \
...
```javascript Node
await fetch("https://dev-agentic-os-aks.setoo.work/api/v1/agents", {
...
The API Playground offers the following service base URLs:

| Service | Base URL | Scope |
| --- | --- | --- |
| **Platform Core** | `https://dev-agentic-os-platform-core-api-aks.setoo.work/api/v1` | Profile, Wallet, Billing, Analytics, Integrations |
| **Comms Engine** | `https://dev-agentic-os-comms-engine-api-aks.setoo.work/api/v1` | Agents, Channels, Calls, Phone Numbers, Knowledge Bases |
| **Campaign Engine** | `https://dev-agentic-os-campaign-engine-api-aks.setoo.work/api/v1` | Voice Campaigns, Contact Lists, WhatsApp, SMS Campaigns |
| **Local Gateway** | `http://localhost:8787/api/v1` | Local development (via the dev gateway below) |

<Callout type="info">
  Select the appropriate engine server from the server dropdown in the interactive playground depending on the endpoint you are testing.
</Callout>
...
`https://dev-agentic-os-aks.setoo.work/api/v1` to the top of the `api.mdx.server` list in
