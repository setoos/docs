...
The API Playground uses the following base URL:

| Server | Description |
| --- | --- |
| `https://dev-agentic-os-aks.setoo.work/api/v1` | Live Cloud API / Production |
| `http://localhost:8787/api/v1` | Local development (via the dev gateway below) |

<Callout type="info">
  Requests in the interactive playground target `https://dev-agentic-os-aks.setoo.work/api/v1` by default. For local multi-service development, you can use the dev gateway script below.
</Callout>
