...
Every request below uses the production base URL `https://dev-agentic-os-aks.setoo.work` and Bearer
authentication.
...
```bash curl
curl https://dev-agentic-os-aks.setoo.work/api/v1/billing/wallet \
...
resp = requests.get(
    "https://dev-agentic-os-aks.setoo.work/api/v1/billing/wallet",
...
```javascript Node
const resp = await fetch("https://dev-agentic-os-aks.setoo.work/api/v1/billing/wallet", {
...
```bash curl
curl -X POST https://dev-agentic-os-aks.setoo.work/api/v1/agents \
  -H "Authorization: Bearer $AGENTIC_OS_API_KEY" \
...
```javascript Node
const resp = await fetch("https://dev-agentic-os-aks.setoo.work/api/v1/agents", {
  method: "POST",
  headers: {
...
```bash
curl https://dev-agentic-os-aks.setoo.work/api/v1/voice-numbers \
  -H "Authorization: Bearer $AGENTIC_OS_API_KEY"
```
...
```bash curl
curl -X POST https://dev-agentic-os-aks.setoo.work/api/v1/channels/voice/call \
  -H "Authorization: Bearer $AGENTIC_OS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```python Python
resp = requests.post(
    "https://dev-agentic-os-aks.setoo.work/api/v1/channels/voice/call",
    headers={"Authorization": f"Bearer {os.environ['AGENTIC_OS_API_KEY']}"},
    json={
        "agentId": "YOUR_AGENT_ID",
        "voiceNumberId": "YOUR_VOICE_NUMBER_ID",
...
```javascript Node
const resp = await fetch("https://dev-agentic-os-aks.setoo.work/api/v1/channels/voice/call", {
  method: "POST",
  headers: {
    Authorization: `Bearer ${process.env.AGENTIC_OS_API_KEY}`,
    "Content-Type": "application/json",
...
```bash curl
curl https://dev-agentic-os-aks.setoo.work/api/v1/call-logs/YOUR_SESSION_ID/full \
  -H "Authorization: Bearer $AGENTIC_OS_API_KEY"
```

```python Python
resp = requests.get(
    f"https://dev-agentic-os-aks.setoo.work/api/v1/call-logs/{session_id}/full",
    headers={"Authorization": f"Bearer {os.environ['AGENTIC_OS_API_KEY']}"},
)
...
const resp = await fetch(
  `https://dev-agentic-os-aks.setoo.work/api/v1/call-logs/${sessionId}/full`,
  { headers: { Authorization: `Bearer ${process.env.AGENTIC_OS_API_KEY}` } },
);
...
BASE="https://dev-agentic-os-aks.setoo.work/api/v1"
