...
Use an existing agent, or create one on the fly.
...
curl -X POST https://api.agentic-os.com/api/v1/agents \
  ...
  -d '{
    ...
    "agent_prompt": "You are a friendly assistant making a short test call. Keep every reply under two sentences.",
...
Save the returned `id` — you'll pass it as `agentId` in the next step.
...
AGENT_ID=$(curl -s -X POST "$BASE/agents" -H "$AUTH" -H "Content-Type: application/json" \
  -d '{"name":"Quickstart Agent","agent_prompt":"Be brief and friendly.","llm_model_id":"gpt-4-turbo","is_active":true}' \
  | jq -r '.id')

SESSION_ID=$(curl -s -X POST "$BASE/channels/voice/call" -H "$AUTH" -H "Content-Type: application/json" \
  -d "{\"agentId\":\"$AGENT_ID\",\"to\":\"+919876543210\",\"from\":\"+1XXXXXXXXXX\",\"provider\":\"PLIVO\"}" \
  | jq -r '.sessionId')
